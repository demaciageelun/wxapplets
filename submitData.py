...
submit_url = "http://58.208.85.67/k3cloud/Kingdee.BOS.WebApi.ServicesStub.DynamicFormService.Submit.common.kdsvc"
...
# 获取表名和数据，写入erp
# data 格式{"Numbers": ["id"]
def submitToErp(formId, data):
    data = {"FormID": formId, "Data": json.dumps(data)}
    response = requests.post(url=submit_url, data=data, cookies=login())
    print(response.text)
